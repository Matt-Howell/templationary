...
export default function GetPage({ data, widgetID }) {
  ...
  return (
    <div>
      <Head>
        ...
        <meta property="og:url" content={`https://templationary.com/get/widgets/${widgetID}`}/>
        ...
        <meta property="twitter:url" content={`https://templationary.com/get/widgets/${widgetID}`}/>
        ...
        <link rel="canonical" href={`https://templationary.com/get/widgets/${widgetID}`}/>
                ...
                <Flex justifyContent={"start"} width="100%" flexDirection={{ base:"column", md:"row" }} maxWidth={"1392px"}>
                    <Flex flexDirection={"column"} justifyContent={"start"} width={{ base:"100%", md:"70%" }}>
                        <Breadcrumb spacing='8px' separator={<FaChevronRight size={12} opacity='0.8' color='gray.500' />}>
                            <BreadcrumbItem>
                                <BreadcrumbLink href={`/widgets/${widgetID}`} _hover={{ textDecoration:"none", opacity:0.9 }} opacity={0.8}>{data.title.split(" ")[0] + " " + data.title.split(" ")[1]}...</BreadcrumbLink>
                            </BreadcrumbItem>
                        ...
                        We make widgets so that they can be used by everyone, for free! It is very simple to add a Notion widget to a to one of your pages, just follow these 4 steps below to start using your new Notion widget in seconds:
                        </Text>
                        <UnorderedList spacing={3} mt={4} fontSize={'1.075rem'} lineHeight={1.65}>
                            ...
                            <ListItem display={'flex'} alignItems="center" listStyleType={'none'}>
                                ...
                                Copy the link sent in the email.
                            </ListItem>
                            <ListItem display={'flex'} alignItems="center" listStyleType={'none'}>
                                <ListIcon as={RiNumber4} fontSize="md" opacity={0.8} />
                                Add a widget in Notion with this URL.
                            </ListItem>
                        </UnorderedList>
                        <form onSubmit={async (e) => {
                            e.preventDefault();
                            const response = await fetch(`https://templationary.com/send?type=widgets&email=${userEmail.current.value}&id=${widgetID}`)
                            ...
                            <InputRightAddon minHeight={65} p={0}><><Button height={"100%"} type="submit" width={"100%"} colorScheme={'blue'} rightIcon={<FaArrowRight />}  borderRadius={"0 0.375rem 0.375rem 0!important"}>Get This Widget</Button></></InputRightAddon>
                        </InputGroup></form>
                    </Flex>
